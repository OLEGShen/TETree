...
class RiderLLMAgent(LLMAgent):
    def __init__(self, role_param_dict):
        ...
        super().__init__('R',has_chat_history=False,online_track=False,json_format=True,system_prompt = '',llm_model='ChatGPT')
    ...
    def decide_time(self,runner_step,info):
        ...
        {rational_or_sensible_prompt}
         ...
         You should think step by step.
          - When you speak, you must use the following format in json:
              {{
              "chain_of_thought":,Your thought chain,  You may consider using the following keywords in 
                              your thought process.You can only select one keyword within each bracket.: 
                              your personality, To earn (more money,more rest), 
                              (Lucrative, Comfortable, Balanced), (Stimulating, Flexible, Fixed) (Increase, decrease, unchanged) working hours. ,
        ...
        try:
            if self.SR_CoT:
                ...
                print(param_dict)
                response_rational = self.get_response(time_prompt, input_param_dict=param_dict)
                param_dict['rational_or_sensible_prompt'] = sensible_prompt
                param_dict.update({'personality': self.personality})
                print(param_dict)
                response_sensible = self.get_response(time_prompt, input_param_dict=param_dict)
                ...
                self.llm_thought_log(runner_step, param_dict, ration_thought=response_rational.get('chain_of_thought'),
                                     sensibility_thought=response_sensible.get('chain_of_thought'), result=result)
                return int(time_int1[0]),int(time_int2[0])
            else:
                print(param_dict)
                response = self.get_response(time_prompt,input_param_dict=param_dict)
                ...
                self.llm_thought_log(runner_step, param_dict, mixed_thought=response.get('chain_of_thought'),result=result)
                return int(time_int1[0]),int(time_int2[0])
            ...
            self.llm_thought_log(runner_step, param_dict, mixed_thought='LLM 选择工作时间失败！',result=result)
    ...
    def take_order(self,runner_step, info):
        order_prompt = '''
                        ...
                        You should think step by step.
                          - When you speak, you must use the following format in json:
                              {{
                              "chain_of_thought":,You may consider using the following keywords in 
                              your thought process.You can only select one keyword within each bracket.: 
                              your personality, To get(more money,more rest), (Profit-driven,Indifferent)
                              choose (more,less) orders ,
        ...
        try:
            if self.SR_CoT:
                ...
                self.llm_thought_log(runner_step, param_dict, mixed_thought=response.get('chain_of_thought'),
                                     result=final_result)
                return final_result
        except Exception as e:
            print(e)
            ...
            return []
    def llm_thought_log(self, runner_step, param_dict, ration_thought='',sensibility_thought='',mixed_thought='',result=None):
...
if __name__ == '__main__':
    ...
    up,sleep = agent.decide_time(10,time_info)